...
contract DssDeployTest is DssDeployTestBase {
    ...
    function testClip() public {
        ...
        this.file(address(dog), "Hole", rad(1000 ether)); // 1000 USDV max on auction
        this.file(address(dog), "COL2", "hole", rad(1000 ether)); // 1000 USDV max on auction
        this.file(address(dog), "COL2", "chop", WAD);
        col2.mint(1 ether);
        col2.approve(address(col2Join), uint(-1));
        col2Join.join(address(this), 1 ether);
        vat.frob("COL2", address(this), address(this), address(this), 1 ether, 20 ether); // Maximun USDV generated
    ...
    function testFireESM() public {
        ...
        esm.fire();
        esm.deny(address(vlxFlip));
        esm.deny(address(col2Clip));
        assertEq(vat.wards(address(pause.proxy())), 0);
        assertEq(vlxFlip.wards(address(pause.proxy())), 0);
    ...
    function testAuth() public {
        ...
        assertEq(vat.wards(address(vlxJoin)), 1);
        ...
        assertEq(vlxFlip.wards(address(pause.proxy())), 1);
        assertEq(vlxFlip.wards(address(esm)), 1);
